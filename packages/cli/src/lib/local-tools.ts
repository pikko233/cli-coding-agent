...
const MAX_FILE_SIZE = 10000; // 上限一万个字符
const MAX_RESULTS = 200;
const MAX_MATCHES = 50;
const MAX_OUTPUT = 20000;
const DEFAULT_TIMEOUT = 30_000; // 30秒超时
...
export async function executeLocalTool(
  ...
  switch (toolName) {
    ...
    case "listDirectory": {
      ...
      const entries = await readdir(resolved); // 该目录下的所有文件和文件夹的名字列表（数组）
      const results: { name: string; type: "directory" | "file" }[] = [];
